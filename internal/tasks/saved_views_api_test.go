...
func TestAPIV1SavedViewsCRUD(t *testing.T) {
	create := performSavedViewRequest(t, http.MethodPost, "/api/v2/saved-views", `{
		"name": "  Overdue work  ",
		"filter": {
			"project": "1",
			"status": "incomplete",
	...
	if location := create.Header().Get("Location"); location != fmt.Sprintf("/api/v2/saved-views/%d", created.ID) {
		t.Fatalf("unexpected Location header %q", location)
	}

	duplicate := performSavedViewRequest(t, http.MethodPost, "/api/v2/saved-views",
	...
	list := performSavedViewRequest(t, http.MethodGet, "/api/v2/saved-views", "", 1, "user@example.com")
	...
	otherUserList := performSavedViewRequest(t, http.MethodGet, "/api/v2/saved-views", "", 2, "other@example.com")
	...
	for index := 1; index < storage.MaxSavedViewsPerUser; index++ {
		response := performSavedViewRequest(t, http.MethodPost, "/api/v2/saved-views",
			fmt.Sprintf(`{"name":"View %d","filter":{}}`, index), 1, "user@example.com")
		assertSavedViewStatus(t, response, http.StatusCreated)
	}
	overLimit := performSavedViewRequest(t, http.MethodPost, "/api/v2/saved-views",
